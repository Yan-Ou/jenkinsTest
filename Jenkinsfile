    ...
    stage('Test') {
      steps {
        container('toolbox'){
          ...
          sh 'v2=$(git diff HEAD^ HEAD tidb_download_pkg.json | grep "^\\+\\s" | grep -o \'"v[0-9]{1,2}.*"\' | sed \'s/"//g\') && echo $v2'
          // sh 'echo $versions'
          ...
          sh 'git diff -U0 HEAD^ HEAD tidb_download_pkg.json | grep "^\\+\\s" | grep -o \'"v\\d{1,2}\\.\\d{1,2}\\.\\d{1,2}$*"\' | sed \'s/"//g\' | xargs -I{} sh -c \'docker pull pingcap/tidb:{} && docker tag pingcap/tidb:{} gcr.io/pingcap-public/tidb:{} && docker push gcr.io/pingcap-public/tidb:{}\''
